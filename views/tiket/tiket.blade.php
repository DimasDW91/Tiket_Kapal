...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            <a href="{{ route('tiket.create') }}" class="btn btn-success btn-sm">
                                <i class="fas fa-plus fa-sm"></i> Tambah Tiket
                            </a>
                        </div>
                    </div>
                    @endif
                                            ...
                                            <tbody>
                                                @foreach ($tiket as $item)
                                                    <tr>
                                                        ...
                                                        <td>{{ optional($item->pemesanan)->kode_booking ?? '-' }}</td>
                                                        <td>
                                                            @php $jadwal = optional($item->pemesanan)->jadwal; @endphp
                                                            @if($jadwal)
                                                        ...
                                                        <td>
                                                            @php
                                                                $badgeClass = match($item->status_tiket) {
                                                                    'aktif' => 'badge-success',
                                                            ...
                                                            @endphp
                                                            <span class="badge {{ $badgeClass }}">{{ ucfirst($item->status_tiket) }}</span>
                                                        </td>
                                                        <td>
                                                            @if($item->sudahDicetak())
                                                                <span class="badge badge-success"><i class="fas fa-check"></i> Sudah</span>
                                                            @else
                                                                <span class="badge badge-warning">Belum</span>
                                                            @endif
                                                        </td>
                                                        <td>{{ optional($item->waktu_cetak)->format('d/m/Y H:i') ?? '-' }}</td>
                                                        <td>

                                                            @if(!$item->sudahDicetak())
                                                            <!-- Tombol Cetak -->
                                                            <form action="{{ route('tiket.cetak', $item->id) }}" method="POST" class="d-inline">
                                                                @csrf
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Apakah Anda yakin ingin menghapus tiket <strong id="kodeTiket"></strong>?
                </div>
    ...
    <script>
        // Isi form hapus & kode tiket sesuai baris yang dipilih
        $('#modalHapus').on('show.bs.modal', function (event) {
            ...
            form.action = "{{ url('tiket') }}/" + id;

            document.getElementById('kodeTiket').textContent = kode;
        });
    </script>
