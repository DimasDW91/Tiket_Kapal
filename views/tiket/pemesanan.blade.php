...
<body id="page-top">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            <a href="{{ route('pemesanan.create') }}" class="btn btn-success btn-sm">
                                <i class="fas fa-plus fa-sm"></i> Tambah Pemesanan
                            </a>
                        </div>
                    </div>
                    @endif
                                            ...
                                            <tbody>
                                                @foreach ($pemesanan as $item)
                                                    <tr>
                                                        ...
                                                        <td>{{ $item->nama_pemesan_display }}</td>
                                                        <td>
                                                            @if($item->jadwal)
                                                                {{ $item->jadwal->pelabuhanAsal->nama_pelabuhan ?? '-' }}
                                                                &rarr;
                                                                {{ $item->jadwal->pelabuhanTujuan->nama_pelabuhan ?? '-' }}
                                                            @else
                                                                -
                                                            @endif
                                                        </td>
                                                        <td>{{ optional($item->tanggal_pesan)->format('d/m/Y H:i') }}</td>
                                                        <td>{{ $item->jumlah_tiket }}</td>
                                                        <td>Rp {{ number_format($item->total_harga, 0, ',', '.') }}</td>
                                                        <td>{{ ucfirst($item->channel) }}</td>
                                                        <td>
                                                            @php
                                                                $badgeClass = match($item->status_pemesanan) {
                                                            ...
                                                            @endphp
                                                            <span class="badge {{ $badgeClass }}">{{ ucfirst($item->status_pemesanan) }}</span>
                                                        </td>
                                                        @if(Auth::user()->role === 'admin')
                                                        <td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Apakah Anda yakin ingin menghapus pemesanan <strong id="kodeBooking"></strong>?
                    Data yang sudah memiliki pembayaran/tiket tidak dapat dihapus.
                </div>
    ...
    <script>
        // Isi form hapus & kode booking sesuai baris yang dipilih
